...
public sealed class EvolutionDashboardService : IAsyncDisposable
{
    ...
    public EvolutionDashboardService(EvolutionService evolution, string model, string? address = null) { this.evolution = evolution; this.model = model; Address = address ?? Url; listener.Prefixes.Add(Address); }
    ...
    async Task Respond(HttpListenerContext context, CancellationToken ct)
    {
        try
        {
            ...
            else if (path is "/api/evolution" or "/api/evolution/history" or "/api/dataset/stats" or "/api/memory" or "/api/recovery" or "/api/activity")
            {
                ...
                object payload = path switch { "/api/evolution/history" => snapshot.History, "/api/dataset/stats" => new { snapshot.DatasetCompleted, snapshot.DatasetRecovery, snapshot.DatasetRejected, snapshot.DatasetReview, snapshot.AverageQualityScore, snapshot.QualityEvaluated, snapshot.LegacyWithoutQualityScore }, "/api/memory" => new { snapshot.Memories }, "/api/recovery" => new { snapshot.Failures, snapshot.RecoveredFailures }, "/api/activity" => snapshot.RecentActivity, _ => snapshot };
...
fetch('/api/evolution').then(r=>r.json()).then(s=>{version.textContent=s.AgentVersion+' · '+s.Model;let q=s.QualityEvaluated>0?(s.AverageQualityScore/10).toFixed(1)+'/10 · base '+s.QualityEvaluated:'sem avaliações';let items=[['Tarefas concluídas',s.TasksCompleted],['Tarefas falhas',s.TasksFailed],['Taxa de sucesso',s.SuccessRate+'%'],['Autonomia',s.AutonomyRate+'%'],['Recovery success',s.RecoverySuccessRate+'%'],['Builds finais quebrados',s.FinalBrokenBuilds],['Experiências',s.Experiences],['Memórias',s.Memories],['Dataset',s.DatasetCompleted],['Quality médio',q],['Ciclos de treino',s.TrainingCycles],['Treinos concluídos',s.TrainingCompleted+'/'+s.TrainingTasks],['Auto-reparos',s.RepairDecisions],['Reparos aprovados',s.RepairsAccepted],['Reparos promovidos',s.RepairsPromoted],['Reversões',s.RepairsReverted]];cards.innerHTML=items.map(x=>`<div class=card><span class=muted>${x[0]}</span><div class=value>${x[1]}</div></div>`).join('');skills.innerHTML=s.Skills.map(x=>`<div class=skill><span>${x.Name}</span><span>${x.Successes}/${x.Attempts} · ${x.SuccessRate}%</span><div class=bar><div class=fill style="width:${x.SuccessRate}%"></div></div></div>`).join('');dataset.innerHTML=`<p>Completed: ${s.DatasetCompleted}</p><p>Recovery: ${s.DatasetRecovery}</p><p>Rejected: ${s.DatasetRejected}</p><p>Review: ${s.DatasetReview}</p><p>Avaliados: ${s.QualityEvaluated}</p><p>Legado sem score: ${s.LegacyWithoutQualityScore}</p><p class=muted>${s.DatasetVersion}</p>`;activity.innerHTML=s.RecentActivity.length?s.RecentActivity.map(x=>{let d=document.createElement('div');d.className='activity';d.textContent=x;return d.outerHTML}).join(''):'<p class=muted>Nenhuma atividade registrada.</p>';draw(s)});function draw(s){let c=brain,x=c.getContext('2d'),n=Math.min(80,8+s.Experiences+s.Memories),pts=[];for(let i=0;i<n;i++){let a=i*2.399,r=20+Math.sqrt(i)*20;pts.push([350+Math.cos(a)*r,155+Math.sin(a)*r*.55])}x.strokeStyle='#334155';pts.forEach((p,i)=>{for(let j=i+1;j<Math.min(n,i+4);j++){x.beginPath();x.moveTo(...p);x.lineTo(...pts[j]);x.stroke()}});x.fillStyle='#ffc400';pts.forEach(p=>{x.beginPath();x.arc(...p,2.5,0,7);x.fill()})}</script></body></html>
""";
}
