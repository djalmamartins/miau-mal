...
fetch('/api/evolution').then(r=>r.json()).then(s=>{version.textContent=s.AgentVersion+' · '+s.Model;let items=[['Tarefas concluídas',s.TasksCompleted],['Tarefas falhas',s.TasksFailed],['Taxa de sucesso',s.SuccessRate+'%'],['Experiências',s.Experiences],['Memórias',s.Memories],['Recoveries',s.RecoveredFailures],['Dataset',s.DatasetCompleted],['Quality médio',s.AverageQualityScore.toFixed(1)]];cards.innerHTML=items.map(x=>`<div class=card><span class=muted>${x[0]}</span><div class=value>${x[1]}</div></div>`).join('');skills.innerHTML=s.Skills.map(x=>`<div class=skill><span>${x.Name}</span><span>${x.Successes}/${x.Attempts} · ${x.SuccessRate}%</span><div class=bar><div class=fill style="width:${x.SuccessRate}%"></div></div></div>`).join('');dataset.innerHTML=`<p>Completed: ${s.DatasetCompleted}</p><p>Recovery: ${s.DatasetRecovery}</p><p>Rejected: ${s.DatasetRejected}</p><p>Review: ${s.DatasetReview}</p><p class=muted>${s.DatasetVersion}</p>`;activity.innerHTML=s.RecentActivity.length?s.RecentActivity.map(x=>`<div class=activity>${x}</div>`).join(''):'<p class=muted>Nenhuma atividade registrada.</p>';draw(s)});function draw(s){let c=brain,x=c.getContext('2d'),n=Math.min(80,8+s.Experiences+s.Memories),pts=[];for(let i=0;i<n;i++){let a=i*2.399,r=20+Math.sqrt(i)*20;pts.push([350+Math.cos(a)*r,155+Math.sin(a)*r*.55])}x.strokeStyle='#334155';pts.forEach((p,i)=>{for(let j=i+1;j<Math.min(n,i+4);j++){x.beginPath();x.moveTo(...p);x.lineTo(...pts[j]);x.stroke()}});x.fillStyle='#ffc400';pts.forEach(p=>{x.beginPath();x.arc(...p,2.5,0,7);x.fill()})}</script></body></html>
""";
}
